parameters <- createParDef(list(n=c(500),xs=c(10,100), beta0=1,beta1=-2,beta2=3, sigmaV=3, sigmaU=5))

sfa.dgp <- function(){
  formula <- as.formula("y ~ X1 + X2")
  beta<-c(beta1,beta2)
  k <- length(beta)
  X <- matrix(runif(n*k,0,xs),n, k)
...
sfa.hnormal.estimator.wrapper <- function(d){
  est <- sfa.hnormal.estimator(d$formula,d$data)
...
ezsim_sfa<-ezsim(
  ...
  dgp           = sfa.dgp,
  estimator     = sfa.estimator.wrapper,
